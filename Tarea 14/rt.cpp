...
Vector sampleBSDF(Vector &n, double &theta, double &phi){
	Vector s, t;
	coordinateSystem(n, s, t);
	Vector wi = makeVec(theta, phi, "direction");
	Vector wiglob = globalizeCoord(wi, n, s, t);
	return wiglob;
}

Vector sampleLight(Vector &wc, double &theta, double &phi){
	Vector s, t;
...
Color shade(const Ray &r, int bounce, int cond) {
	...
	Vector wi = sampleBSDF(n, theta, phi);
	double dotCos = n.dot(wi);
	Color f = BRDF(baseColor);
	Ray newRay = Ray(x, wi);
	colorValue = f.mult(shade(newRay, bounce++, 0)) * (fabs(dotCos)/(prob*continueprob));

	// imluminacion directa
	double t1;
	int id1 = 0;
	Color directLight = Color();
	double theta1, phi1, cosTmax, prob1;
	paramSolidAngle(x, theta1, phi1, cosTmax);
	Vector wc = (spheres[7].p - x).normalize();
	Vector wl = sampleLight(wc, theta1, phi1);
	prob1 = probSolidAngle(cosTmax);
	if (intersect(Ray(x, wl), t1, id1) && id1 == 7){
		double dotCos1 = n.dot(wl);
		prob1 = probSolidAngle(cosTmax);
		directLight = f.mult(spheres[7].e * fabs(dotCos1) * (1.0 / (prob1*continueprob)));
	}

	return obj.e + directLight + colorValue;
}
