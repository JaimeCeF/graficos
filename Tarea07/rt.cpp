...
// funcion caracteristica positiva
bool chiPlus(double &a){
	if (a > 0) return true;
	else return false;
}
...
double NDF(double &NoH, double &roughness){
	if (!chiPlus(NoH)) return 0;
	else {
		double alfa2 = roughness * roughness;
		double theta = acos(NoH);
		double tanTheta = tan(theta);
		double a = exp(-(tanTheta * tanTheta)/alfa2);
		double b = M_PI * alfa2 * pow(NoH, 4.0);
		return a / b;
	}
}

double G1(double &WvWh, double &WvN, double &roughness){
	double a = WvWh / WvN;
	if (!chiPlus(a)) return 0;
	else {
		double theta = acos(WvN);
		double tanTheta = tan(theta);
		double a1 = 1 / (roughness * tanTheta);
		if (a <1.6) return ((3.535*a1 + 2.181*a1*a1) / (1 + 2.276*a + 2.577*a1*a1));
		else return 1;
	}
}

// funcion para determinar BRDF Microfacet
Color BRDFmicro(Vector &wi, Vector &wo, Vector &n, Color f) {
	Vector F;
	double D;
	double G;
	double prob;
	...
	double NoV = n.dot(wo);
	double NoH = n.dot(wh);
	double VoH = wo.dot(wh);
	double LoH = wi.dot(wh);

	D = NDF(NoH, roughness);
	G = G1(VoH, NoV, roughness) * G1(LoH, NoL, roughness);
	prob = D * (NoH / (4.0*fabs(VoH)));

	Color diffuse = f * invPi;
	// Color brdf = (F * D * G) * (1.0/(4 * NoL * NoV));
	Color brdf = G * (1.0/prob);
	return brdf;
}

// funcion de estimador Monte Carlo para muestreo coseno hemisferico
Vector monteCosHem(int &N, Vector &n, Point &x, Color f){
	Color L, sum;
	Vector s, t;
...
Vector monteCarloArea(int &N, Vector &n, Point &x, Color f){
	Color L, sum;
	Vector s, t;
	double theta, phi, prob;
	coordinateSystem(n, s, t);
...
Vector monteCarloSolidAngle(int &N, Vector &n, Point &x, Vector &wo, Color f){
	Color L, sum;
	Vector s, t;
...
Color shade(const Ray &r) {
	...
	// else colorValue = monteCosHem(N, n, x, baseColor)*(1.0/N);
	// else colorValue = monteCarloArea(N, n, x, baseColor)*(1.0/N);
	else colorValue = monteCarloSolidAngle(N, n, x, wo, baseColor)*(1.0/(N));
